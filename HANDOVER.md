...
| ~~🟡 中~~ | ~~启动时 WebView 白屏闪烁~~ | ✅ 2026-08-27 已修复：`tauri.conf.json` `app.windows[].backgroundColor = "#0f1419"` 让原生 WebView2 在 HTML/CSS 加载**之前**就显示深色背景。Dark 用户完全无白屏；light 用户仍有 0.5 秒"深→浅"过渡（Tauri 2 的 backgroundColor 是静态值不支持 system theme media query）。修复要点（子代理研究 + 验证）：tauri 2.11.5 `WebviewWindowBuilder::background_color(Color)` → wry `with_background_color(RGBA)`；Windows 8+ alpha channel 被忽略（除非 0），所以用不透明 hex。 |
